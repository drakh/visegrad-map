...
(function ()
{
	...
	var parse = function (ua, platform)
	{
		...
		var UA = ua.match(/(opera|ie|firefox|chrome|trident|crios|version)[\s\/:]([\w\d\.]+)?.*?(safari|(?:rv[\s\/:]|version[\s\/:])([\w\d\.]+)|$)/) || [
				null,
				'unknown',
				0
			];
...
Form.Validator.Inline = new Class({
	...
	watchFields: function (fields)
	{
		fields.each(function (el)
		{
			...
			if (this.options.evaluateFieldsOnChange)
			{
				el.addEvent('change', this.validationMonitor.pass([
					el,
					true,
					this.options.scrollToErrorsOnChange
				], this));
			}
		}, this);
	}
...
(function ()
{
	...
	this.Tips = new Class({
		...
		{
			this.attached_elements = elements;
...
Locale.define('pl-PL', 'Date', {
	...
	ordinal: function (dayOfMonth)
	{
		return (dayOfMonth > 3 && dayOfMonth < 21) ? 'ty' : [
			'ty',
			'szy',
			'gi',
			'ci',
...
(function ()
{
	...
	Locale.define('ru-RU', 'Date', {
		...
		monthsAgo: function (delta)
		{
			return '{delta} ' + pluralize(delta, 'месяц', 'месяца', 'месяцев') + ' назад';
		},
		yearAgo: 'год назад',
		yearsAgo: function (delta)
		{
		...
		monthsUntil: function (delta)
		{
			return 'через {delta} ' + pluralize(delta, 'месяц', 'месяца', 'месяцев') + '';
		},
		yearUntil: 'через',
		yearsUntil: function (delta)
		{
...
(function ()
{
	...
	Locale.define('sk-SK', 'Date', {
		...
		minutesAgo: function (delta)
		{
			return 'pred {delta} ' + pluralize(delta, 'minútou', 'minútami', 'minútami');
		},
		...
		weeksAgo: function (delta)
		{
			return 'pred {delta} ' + pluralize(delta, 'týždňom', 'týždňami', 'týždňami');
		},
